...
patchResetActionSequence = [(RELEASE, RELEASE2)] * 4
...
while True:
    ...
    while True:
        user_input = input("Enter 'c' for control, 'e' for empty, 'm' for metal, 'r' for release: ")
        location_input = input("Enter Location number: ")
        ...
        if(user_input == 'c'):
            ...
            file_id = int(time.time()%100000)
            with open(f'../data_pipeline/rssi_history_loc{location_input}_control_{file_id}.txt', 'w') as f:
                for item in rssi_history:
                    f.write("%s\n" % item)
                
                f.write("%s\n" % total_time)
            
            print("Recording stable rssi")
            rssi_history = []
            for i in range(10):
                rssi, data_id_buff = pm.get_rssi_from_wifi_board(sock, addr, data_id_buff)
                rssi_history.append(rssi)
                ...
                sleep(1)
            file_id = int(time.time()%100000)
            with open(f'../data_pipeline/rssi_history_loc{location_input}_metamorph_stable_{file_id}.txt', 'w') as f:
                for item in rssi_history:
                    f.write("%s\n" % item)
                
                rssi_history = np.array(rssi_history)
                f.write("%s\n" % np.mean(rssi_history))
        ...
        if user_input == 'r':
            ...
            pm.serial_send_syn(dev2, actionHold)
            print("reset done")
            rssi_history = []
            continue
            ...
            if user_input == 'e':
                ...
                name = "metal"
            with open(f'../data_pipeline/rssi_history_loc{location_input}_{name}_{file_id}.txt', 'w') as f:
                for item in rssi_history:
                    f.write("%s\n" % item)
