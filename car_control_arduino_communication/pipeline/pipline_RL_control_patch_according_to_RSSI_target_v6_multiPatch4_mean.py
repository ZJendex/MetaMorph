...
class QLearningAgent:
    ...
    def get_action(self, state):
        ...
        print(f"not HOLD, get action epsilon is {self.epsilon}")
...
env = MyEnvironment(target_value=-50, initial_value=rssi)
...
while True: 
    ...
    if test == 1:
        ...
        if pre_train_cur == pre_train_num: # end of pre_train
            ...
            # target = int(max(RSSI_reset) + goal) 
            if goal == 1:
                target = int(max(RSSI_reset))
            else:
                target = int(min(RSSI_reset))
        ...
        agent.target_rssi = env.target_value
        # RL learning
        if action[0] != -1:
