...
# goal config
goal = 1 # 1 for highest, -1 for lowest
...
def signal_handler(sig, frame):
    global total_rewards, accuracy, RSSI_ALL
    ...
    ax2 = ax1.twinx()
    color = 'tab:blue'
    ax2.set_ylabel('Dis to Target', color=color)
    ...
    ax3 = ax1.twinx()
    color = 'tab:green'
    ax3.set_ylabel('RSSI', color=color)
    ax3.plot(RSSI_ALL, color=color)
    ...
    id = time.time()
    plt.title("Rewards, Accuracy and RSSI Changes over Steps")
    plt.grid(True)
...
class QLearningAgent:
    ...
    def get_action(self, state):
        ...
        if np.random.uniform(0, 1) < self.epsilon:
            ...
            print(f"EXPLOR flatten index is {flat_index}")
            return flat_index
        else:
...
while True: 
    ...
    if test == 1: # test mode
        ...
        if pre_train_cur == pre_train_num: # end of pre_train
            ...
            pre_train = 0
            target = max(RSSI_reset) + goal # goal is 1 for highest, -1 for lowest
